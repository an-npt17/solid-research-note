...
const PROVIDERS = [
	{
		label: "solidcommunity.net (free, public)",
			...
			<div className="bg-white rounded-2xl shadow-lg p-10 w-full max-w-md">
				...
				<form onSubmit={handleLogin} className="space-y-4">
					<div>
						...
						/>
						{provider &&
							toIssuerRoot(provider) !== provider.trim() && (
								<p className="mt-1 text-xs text-amber-600">
									Will use server root:{" "}
									<strong>{toIssuerRoot(provider)}</strong>
								</p>
							)}
					</div>
